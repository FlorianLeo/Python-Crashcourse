...
class AlienInvasion:
	...
	def __init__(self):
		...
		# Make the instance from Statistics to have statistic available, and
		# and instance from the scoreboard
	...
	def _check_play_button(self, mouse_pos):
		...
		# button_clicked works like a flag and stores True or False only
		# The game wil restart only if Play is clicked and the game is not currently active
		button_clicked = self.play_button.rect.collidepoint(mouse_pos)
		if button_clicked and not self.stats.game_active:
			# Reset the game statistics and restart the game
			self.settings.initialize_dynamic_settings()
			self.stats.reset_stats()
			self.stats.game_active = True
			self.sb._scores_image_and_rectangle()

			# Get rid of any remaining bullets and/or aliens
			self.aliens.empty()
			self.bullets.empty()

			# Create a new fleet and a new ship
			self._create_fleet_of_aliens()
			self.ship.center_ship()

			# If game is in play hide the mouse arrow
	...
	def _check_bullet_alien_collision(self):
		...
		# Scoring each bullet that hits an alien
		if collisions:
			for aliens in collisions.values():
				self.stats.score += self.settings.alien_points * len(aliens)
			self.sb._scores_image_and_rectangle()
		# if there are no aliens - create them
		if not self.aliens:
			self.bullets.empty()
			self._create_fleet_of_aliens()
