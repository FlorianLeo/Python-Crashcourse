...
class Starsgame:
	...
	def _create_fleet_of_stars(self):
		...
		# which was calculated in line 39
		for current_star_number in range(total_of_stars_possible_in_x):
			# Create the star by calling the helper-method to create a star and place it in the row after creation
			self._create_star(current_star_number)

	def _create_star(self, current_star_number):
		"""Create a single star and place it in the row."""
		# Create the star
		star = Star(self)
		star_width = star.rect.width
		star.x = star_width + 2 * star_width * current_star_number
		star.rect.x = star.x
		self.stars.add(star)
